 ...
 */
@Service
public class ServiceArchiveTask extends BaseService {
    ...
    public void execute(AbstractCommonData in, AbstractCommonData inHead, AbstractCommonData out, AbstractCommonData outHead) {
        ...
        update("service_archive_hour", time);     //插入快照计算结果
        update("delete_service_flash", time);     //删除快照

        time = DateUtil.addDay(time, ARCHIVE_DAY);
        update("service_archive_day", time);      //归档之前第3~10天的数据
        update("delete_service_hour", time);      //把已归档的日志删除

        if (archiveMonth) {
            //归档月日志
            time = DateUtil.setDay(time, 1);
            time = DateUtil.addMonth(time, ARCHIVE_MONTH);
            update("service_archive_month", time);
            update("delete_service_day", time);
        }
    }
}
